...
try {
	...
	log.error("Unable to load sharp module. See https://sharp.pixelplumbing.com/install");
}
...
class Uploader {
	...
	static routeUploadFile(req, res) {
		...
		busboyInstance.on("file", (fieldname, fileStream, filename, encoding, contentType) => {
			...
			// sharp's prebuilt libvips does not include gif support, but that is not a problem,
			// as GIFs don't support EXIF metadata or anything alike
			const isImage =
				Uploader.removeMetadataAvailable() &&
	...
	static removeMetadataAvailable() {
